...
try:
    ...
    # flag = 7则识别矩形框
    flag = 0
    state_lst = ['normal', 'red_point', 'threshold', '(1)(2)set', '(1)', '(2)', '(3)(4)', 'rect_detect']
...
#    threshold_dict['red_point'] = []
    ...
    def detect_rectangles():
        """检测矩形框的函数"""
        global pin, img, rect_points_lst_3_4
        pin.value(0)  # 关闭激光笔进行矩形检测
        img = sensor.snapshot(chn=CAM_CHN_ID_0)
        img = img.copy(roi=cut_roi)
        ...
        if rects and len(rects) > 0:
            for rect in rects:
                ...
                img.draw_line(corner[1][0], corner[1][1], corner[2][0], corner[2][1], color=(0, 255, 0), thickness=3)
                img.draw_line(corner[2][0], corner[2][1], corner[3][0], corner[3][1], color=(0, 255, 0), thickness=3)
                img.draw_line(corner[3][0], corner[3][1], corner[0][0], corner[0][1], color=(0, 255, 0), thickness=3)
                
                # 计算矩形中心点
                center_x = sum([corner[i][0] for i in range(4)]) / 4
                center_y = sum([corner[i][1] for i in range(4)]) / 4
                img.draw_cross(int(center_x), int(center_y), color=(255, 0, 0), size=10, thickness=2)
        ...
        # 如果检测到两个矩形，计算它们之间的中线路径
        if len(detected_rects) == 2:
            corner_1, corner_2 = order_two_rects(detected_rects[0]['corners'], detected_rects[1]['corners'])
            rect_points_lst_3_4 = [[(x1 + x2)/2, (y1+y2)/2] for (x1, y1), (x2, y2) in zip(corner_1, corner_2)]
            
            # 绘制中线路径
            for i in range(4):
                next_i = (i + 1) % 4
                img.draw_line(int(rect_points_lst_3_4[i][0]), int(rect_points_lst_3_4[i][1]), 
                            int(rect_points_lst_3_4[next_i][0]), int(rect_points_lst_3_4[next_i][1]), 
                            color=(255, 255, 0), thickness=2)
            
            img.draw_string_advanced(10, 10, 25, "检测到{}个矩形".format(len(detected_rects)), color=(0, 255, 0))
        else:
            img.draw_string_advanced(10, 10, 25, "矩形识别错误: {}个".format(len(detected_rects) if rects else 0), color=(255, 0, 0))
